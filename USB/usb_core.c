...
USB_OTG_STS USB_OTG_CoreInitHost(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  for (i = 0; i < pdev->cfg.host_channels; i++)
  {
    ...
    USB_OTG_WRITE_REG32( &pdev->regs.HC_REGS[i]->HCGINTMSK, 0 );	//XXXX
  }
#ifndef USE_OTG_MODE
...
USB_OTG_STS USB_OTG_HC_Init(USB_OTG_CORE_HANDLE *pdev , uint8_t hc_num)
{
  ...
  USB_OTG_HCGINTMSK_TypeDef  hcintmsk;	//XXXX
  ...
  USB_OTG_WRITE_REG32(&pdev->regs.HC_REGS[hc_num]->HCGINTMSK, hcintmsk.d32);	//XXXX
...
/**
* @brief  USB_OTG_EP0StartXfer : Handle the setup for a data transfer for EP0 and
*         starts the transfer
* @param  pdev : Selected device
* @retval USB_OTG_STS : status
*/
  ...
  if (pdev->dev.DevRemoteWakeup) 
  {
    ...
    if(dsts.b.suspsts == 1)
    {
      if(pdev->cfg.low_power)
      {
        ...
        power.d32 = USB_OTG_READ_REG32(pdev->regs.PCGCCTL);			//XXXXXXXXXXXXXXX
        power.b.gatehclk = 0;
        power.b.stoppclk = 0;
        USB_OTG_WRITE_REG32(pdev->regs.PCGCCTL, power.d32);			//XXXXXXXXXXXXXXX
      }   
...
void USB_OTG_UngateClock(USB_OTG_CORE_HANDLE *pdev)
{
  if(pdev->cfg.low_power)
  {
    ...
    {
      /* un-gate USB Core clock */
      power.d32 = USB_OTG_READ_REG32(pdev->regs.PCGCCTL);			//XXXXXXXXXXXXXXXXXX
      power.b.gatehclk = 0;
      power.b.stoppclk = 0;
      USB_OTG_WRITE_REG32(pdev->regs.PCGCCTL, power.d32);			//XXXXXXXXXXXXXXXXXX
